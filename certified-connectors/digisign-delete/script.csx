...
// Description: Custom code for Digisign custom connectors (Delete, Get) to handle Auth-Token exchange and Base64 unified file uploads.
...
public class Script : ScriptBase
{
    ...
    private async Task<HttpResponseMessage> HandleUniversalCallAsync(string baseUrl, string token, CancellationToken cancellationToken)
    {
        try
        {
            ...
            // Restrict methods to only GET and DELETE for delete operations
            if (methodStr != "GET" && methodStr != "DELETE")
            {
                var errorResponse = new HttpResponseMessage(HttpStatusCode.MethodNotAllowed);
                errorResponse.Content = new StringContent($"Method '{methodStr}' is not allowed in this connector. Only GET and DELETE are supported.", Encoding.UTF8, "text/plain");
                return errorResponse;
            }
