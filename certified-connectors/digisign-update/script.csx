...
// Description: Custom code for Digisign custom connectors (Update, Get) to handle Auth-Token exchange and Base64 unified file uploads.
...
public class Script : ScriptBase
{
    ...
    private async Task<HttpResponseMessage> HandleUnifiedUploadAsync(string baseUrl, string token, CancellationToken cancellationToken)
    {
        try
        {
            ...
            string fileContentBase64 = payload["fileContent"]?.ToString();
    ...
    private async Task<HttpResponseMessage> HandleUniversalCallAsync(string baseUrl, string token, CancellationToken cancellationToken)
    {
        try
        {
            ...
            // Restrict methods to only GET, PUT, and PATCH for update operations
            if (methodStr != "GET" && methodStr != "PUT" && methodStr != "PATCH")
            {
                var errorResponse = new HttpResponseMessage(HttpStatusCode.MethodNotAllowed);
                errorResponse.Content = new StringContent($"Method '{methodStr}' is not allowed in this connector. Only GET, PUT, and PATCH are supported.", Encoding.UTF8, "text/plain");
                return errorResponse;
            }
