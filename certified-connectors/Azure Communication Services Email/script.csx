...
public class Script : ScriptBase
{
    ...
    public void AddRepeatabilityHeadersToRequest()
    {
        var date = DateTimeOffset.UtcNow.ToString("r");
        var requestId = Guid.NewGuid().ToString();
        Context.Request.Headers.TryAddWithoutValidation("repeatability-first-sent", date);
    ...
    public async Task PrepareRequestEmailAttachmentsAndHeaders()
    {
        ...
        if (requestContentAsJson.ContainsKey("attachments"))
        {
            var attachments = requestContentAsJson["attachments"];
        ...
        Context.Request.Content = CreateJsonContent(body);
    }
