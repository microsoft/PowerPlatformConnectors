	public class Script : ScriptBase
	{
		...
		private async Task<HttpResponseMessage> HandleForwardOperation(){
			switch (this.Context.OperationId) {
                ...
                case "FindRequest":
                    return await this.HandleFindRequest().ConfigureAwait(false);
			}
		...
		private async Task<HttpResponseMessage> HandleFindRequest(){								//	getting PK Id of the request using Display Id in Find request action
			Uri uri = this.Context.Request.RequestUri;
			var query = HttpUtility.ParseQueryString(uri.Query);
			if(string.Equals(query["IdType"], "Display Id")){
				var updatedURL = Uri.UnescapeDataString(this.getRequestUrl());
				var apiUrl = this.getRequestUrl().Substring(0, updatedURL.IndexOf("?"));
				var listInfo = "{\"list_info\":{\"row_count\":\"1\",\"fields_required\":[\"id\"],\"search_criteria\":[{\"field\":\"display_id\",\"condition\":\"is\",\"value\":\"" + query["Id"] + "\"}]}}";
				
				apiUrl = apiUrl + "?input_data=" + HttpUtility.UrlEncode(listInfo);
				HttpRequestMessage newReq = new HttpRequestMessage(HttpMethod.Get, new Uri(apiUrl));
				CopyHeaders(this.Context.Request, newReq);
				HttpResponseMessage response = await this.Context.SendAsync(newReq, CancellationToken);
				var responseString = await response.Content.ReadAsStringAsync().ConfigureAwait(continueOnCapturedContext: false);
				var requestList = JObject.Parse(responseString.ToString());
				var responseStatus = JArray.Parse(requestList["response_status"].ToString());
				var statusCode = JObject.Parse(responseStatus[0].ToString());
				if(string.Equals(statusCode["status_code"].ToString(), "2000")){
					var requestIdArray = JArray.Parse(requestList["requests"].ToString());
					var requestId = JObject.Parse(JArray.Parse(requestList["requests"].ToString())[0].ToString());
					return await this.HandleFindRequestDetail(requestId["id"].ToString()).ConfigureAwait(false);
				}
			}
			return await this.HandleFindRequestDetail(null).ConfigureAwait(false);
		}

		private async Task<HttpResponseMessage> HandleFindRequestDetail(string pkId){				//	Handling Find request action using PK Id.
            Uri uri = this.Context.Request.RequestUri;
			var query = HttpUtility.ParseQueryString(uri.Query);
			var id = (string.Equals(query["IdType"], "Display Id"))? pkId : query["Id"];
			var updatedURL = Uri.UnescapeDataString(this.getRequestUrl());
            var apiUrl = this.getRequestUrl().Substring(0, updatedURL.IndexOf("?")) + "/" + id;
            Context.Request.RequestUri = new System.Uri(string.Format(apiUrl));
        	HttpResponseMessage response = await this.Context.SendAsync(this.Context.Request, this.CancellationToken).ConfigureAwait(continueOnCapturedContext: false);
			return response;
		}

        private void CopyHeaders(HttpRequestMessage copyFrom, HttpRequestMessage copyTo){   		//	To copy all the headers to new object to make multiple calls in the same operation.
