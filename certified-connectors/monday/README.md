...
- `Update an item's column` - Updates a single column for an item given the item ID. Can also be used to update the item name. If the value is blank, the column value is cleared. Returns the ID of the updated item.
- `Update multiple item columns` - Updates multiple item columns. If the column value is blank, that column is not updated. Returns the updated item ID.
