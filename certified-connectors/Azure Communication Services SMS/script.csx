...
public class Script : ScriptBase
{
    public override async Task<HttpResponseMessage> ExecuteAsync()
    {
        ...
        {
            var connectionString = ExtractTemporaryHeaderValue("Connection-String");
            var resourceEndpoint = ExtractTemporaryHeaderValue("Resource-Endpoint");

            if (!string.IsNullOrWhiteSpace(connectionString))
            {
                await PrepareRequestWithEndpointAndHMAC(connectionString);
            }
            else if (!string.IsNullOrWhiteSpace(resourceEndpoint))
            {
                PrepareRequestWithEndpoint(resourceEndpoint);
            }
            else
            {
                throw new Exception("EXPECTED400: invalid connection");
            }
    ...
    public string ExtractTemporaryHeaderValue(string headerKey)
    {
        var headerValues = Context.Request.Headers.GetValues(headerKey);
        Context.Request.Headers.Remove(headerKey);
        return headerValues.FirstOrDefault<string>();
    }

    public void PrepareRequestWithEndpoint(string resourceEndpoint)
    {
        var endpointUri = new Uri(resourceEndpoint);
     ...
     */
    public async Task PrepareRequestWithEndpointAndHMAC(string connectionString)
    {
