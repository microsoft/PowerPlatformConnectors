...
// Description: Custom code for Digisign custom connectors (List, Get) to handle Auth-Token exchange and Base64 unified file uploads.
...
public class Script : ScriptBase
{
    ...
    private async Task<HttpResponseMessage> HandleUniversalCallAsync(string baseUrl, string token, CancellationToken cancellationToken)
    {
        try
        {
            ...
            // Restrict methods to only GET for list operations
            if (methodStr != "GET")
            {
                var errorResponse = new HttpResponseMessage(HttpStatusCode.MethodNotAllowed);
                errorResponse.Content = new StringContent($"Method '{methodStr}' is not allowed in this connector. Only GET is supported.", Encoding.UTF8, "text/plain");
                return errorResponse;
            }
