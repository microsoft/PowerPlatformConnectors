# Qatar Open Data (Independent Publisher)

Qatar Open Data is the national open data portal of the State of Qatar, published by the National Planning Council. It offers more than 1,600 government datasets covering areas such as population and demography, labour and employment, finance and economy, energy and environment, health, education, trade and industry, transport and infrastructure, and the Sustainable Development Goal indicators, in both English and Arabic. This connector lets Power Automate flows, Power Apps, and Microsoft Copilot Studio agents search the dataset catalogue, read and filter the records inside a dataset, enumerate facet values, list dataset attachments, and export catalogue or dataset content as files, using the Opendatasoft Explore API v2.1 that powers the portal.

## Publisher: Dan Romano

**Stack Owner:** State of Qatar

## Prerequisites

There are no licensing or account prerequisites for this connector. The Qatar Open Data portal is a public, open data service, and every operation in this connector reads data that is published for public use.
...
No credentials are required. The Qatar Open Data portal serves the Explore API v2.1 anonymously, so this connector uses no authentication and the connection is created without entering a key, a user name, or a password.
...
The underlying Opendatasoft platform can also accept an optional API key for access to restricted datasets on portals that publish them. The Qatar Open Data portal exposes its catalogue publicly, so no key is needed, and this connector does not send one.
...
The **Filter**, **Select Fields**, **Group By**, and **Order By** options all accept expressions written in the Opendatasoft Query Language (ODSQL). For example, `year > 2020 AND nationality like 'Qatari'` is a valid **Filter**, and `sum(count) as total` combined with a **Group By** of `year` aggregates records by year. The ODSQL reference is published in the [Opendatasoft Explore API documentation](https://help.opendatasoft.com/apis/ods-explore-v2/).
...
- **Rate limits.** The portal allows 500,000 API calls per day per client, and the quota resets at midnight UTC. The remaining allowance is returned in the `X-RateLimit-Remaining` response header, alongside `X-RateLimit-Limit` and `X-RateLimit-Reset`. When the quota is exhausted, the portal returns HTTP 429. The portal also applies a separate per-dataset allowance, reported in the `X-RateLimit-dataset-*` headers.
...
- **Dataset picker coverage.** The **Dataset ID** field lists the first 100 datasets returned by the catalogue. The portal publishes more than 1,600 datasets, so for a dataset outside the first 100, enter its identifier manually. The identifier appears in the dataset URL, immediately after `/datasets/`.
...
- **Language of metadata.** Dataset titles, descriptions, themes, keywords, publishers, licences, and field labels are published in English and Arabic. The portal returns both versions side by side, so the connector exposes them as separate outputs, for example **Title (English)** and **Title (Arabic)**, alongside the language-neutral **Title** that the portal selects for you. Use the **Language** option to control the language used when formatting values such as dates.
